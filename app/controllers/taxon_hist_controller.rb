class TaxonHistController < ApplicationController
  ...
  def create 
    ...
    if @taxon_hist.save
      ...
      if params[:taxon_hist]
        ...
        @no_right_col = true
        redirect_to :action => 'show_taxonomic_history', :id => @taxon_name, :controller => 'taxon_name'
      else 
        redirect_to :action => 'list'
      end
  ...
  def update
    ...
    if @taxon_hist.update_attributes(params[:taxon_hist])
      ...
      if params[:in_taxon_hists] == 1
        ...
        redirect_to(:action => 'show_taxonomic_history', :controller => 'taxon_name', :id => @taxon_hist.taxon_name_id) 
      end 
    else
      render :action => 'edit'
    end
