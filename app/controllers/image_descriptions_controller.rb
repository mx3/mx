class ImageDescriptionsController < ApplicationController
    ...
    if @image_description.update_attributes(params[:image_description])
      ...
      if params[:update_and_next]
        ...
        if @id
          ...
          redirect_to(:action => 'list', :controller => 'images')
        end
      else
        redirect_to :action => :show, :id => @image_description.image.id, :controller => :images
      end
    else
      render :action => 'list', :controller => :images
    end
  end
  ...
  # TODO: make this an AJAX mouseover
  def more
