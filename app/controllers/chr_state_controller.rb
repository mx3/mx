class ChrStateController < ApplicationController
  # used only in redirects of tags/figures

  in_place_edit_for :chr_state, :notes

  def index
    redirect_to :action => :index, :controller => :chr
  end

  def show
    redirect_to :action => :show, :controller => :chr, :id => ChrState.find(params[:id]).chr_id
  end
  ...
  def destroy_phenotype
    ...
    chr_state.save
    redirect_to :controller => :chr, :action => :show, :id => chr_state.chr_id
  end
