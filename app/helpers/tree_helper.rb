...
module TreeHelper
  ...
  # _n
  #  <div style="border-left: 2px solid green; padding-left: 0; position: relative; left: 5px "><%= n.label.to_s %></div>
  ...
  def r1(root_node, txt, depth = 0)
    # returns terminals only
    s = '<div style=" margin-left: 10; border-left: 2px solid black;">' + render(:partial => 'n', :object => root_node)
  ...
  def r2(root_node, txt, depth = 0)
    ...
    s = '<div style=" margin-right: 10;  margin-top: 5px; border: 1px solid green; background-color: rgb(' + ColorHelper::ranged_color(root_node.pct_depth, 'green') + '); margin: 4px; ">' + render(:partial => 'n', :object => root_node)
        depth = depth + 1
      txt = root_node.children.collect{|n| r2(n, s, depth)}.join(" ")
     return s + '<div style=" border-left: 2px solid purple; border-bottom: 2px solid purple; margin-left: 10; position: relative;   ">' +  txt + '</div>'  + '</div>'
  end
