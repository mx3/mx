...
module FiguresHelper
  ...
  def url_for_zoom_figure(figure) # :yields: String (URL)
    ...
    if @server_name == 'development'
      base += '127.0.0.1:3000' +  url_for(:action => :show_zoom, :controller => (pub_str + '/figures'), :id => figure.id)
    else
       base += @server_name +  url_for(:action => :show_zoom, :controller => (pub_str + '/figures'), :id => figure.id)
    end
  end
  ...
  def link_to_figured(figure, link_text = 'show')
    return content_tag(:strong, "ERROR? #{figure.addressable_type}:#{figure.addressable_id}", :style => 'color:red;') if !figure.figured_obj
    link_to(link_text, :action => :show_figures, :id => figure.addressable_id, :controller => figure.addressable_type.underscore.pluralize)
  end
    ...
    if (o)
      content_tag :div, :id => "f_#{o.class.to_s}_#{o.id}", :style => 'display:inline;' do
        content_tag :span, :id => "fl_#{o.class.to_s}_#{o.id}" do
          content_tag(:a, 'Fig', 'data-basic-modal' => '', :href => url_for(:proj_id => o.proj_id, :action => :illustrate, :controller => :figures, :fig_obj_id => o.id, :fig_obj_class => o.class.to_s) )
        end
      end
    end
  end
