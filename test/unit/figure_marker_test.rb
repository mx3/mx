...
class FigureMarkerTest < ActiveSupport::TestCase
  ...
  test "that attributes are added on render" do
    ...
    desired = "<g display='inline' fill='#456' id='marker_#{@fm.id}' opacity='0.3' stroke='#123' stroke-width='2'><path d='M45.146,0'/></g>"
  ...
  test "that attribute DEFAULT_OPACITY and other defaults are used on render" do
    ...
    desired = "<g display='inline' fill='#456' id='marker_#{@fm.id}' opacity='#{FigureMarker::DEFAULT_OPACITY}' stroke='#123' stroke-width='2'><path d='M45.146,0'/></g>"
    assert_equal desired, doc.to_s.gsub(/\n/,'')
  end
  ...
  test "that lines are allowed" do 
    ...
    @fm.save!
    desired = "<line x1='0' x2='200' y1='0' y2='200'/>"
    assert_equal desired, @fm.svg
