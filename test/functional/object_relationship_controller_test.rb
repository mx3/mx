...
class ObjectRelationshipControllerTest < ActionController::TestCase
  def setup
   ...
   test "should create object_relationship" do
     assert_difference('ObjectRelationship.count') do
       post :create, @opts.merge!(:object_relationship => {:interaction => "bars"})
     end
     assert_redirected_to :controller => :object_relationship, :action => :show, :id => assigns(:object_relationship) # @object_relationship # "http://test.host/projects/1/object_relationship/show/" # object_relationship_path(assigns(:object_relationship))
   end
   ...
   test "should update object_relationship" do
     ...
     assert_redirected_to :action => :show, :controller => :object_relationship, :id => @object_relationship.id # assigns(:object_relationship)
   ...
   test "should destroy object_relationship" do
     assert_difference('ObjectRelationship.count', 0) do
       delete :destroy, @opts.merge(:id => @object_relationship.id)
     end
       assert_redirected_to :controller => "object_relationship", :action => "list"  
   end
