...
require 'public/claves_controller'

# Re-raise errors caught by the controller.
class Public::ClavesController; def rescue_action(e) raise e end; end
class Public::ClavesControllerTest < ActionController::TestCase
  
  def setup
    @controller = Public::ClavesController.new
    ...
    assert @person
    @proj_new = Proj.create!(:name => "foo", :unix_name => 'blorf', :public_server_name => 'test.com', :public_controllers => ["claves"] )
  ...
  def test_index
   @pub_base_urls.each do |base_url|
          opts = {:controller => "public/claves", :action => "index", :proj_id => "#{@proj_new.id}"}
          get :index, opts   
  ...
  def test_routes
    @pub_base_urls.each do |base_url|
          ...
          assert_recognizes opts , "projects/#{@proj_new.id}/public/claves"
          assert_routing "projects/#{@proj_new.id}/public/claves", opts
    end
  end
