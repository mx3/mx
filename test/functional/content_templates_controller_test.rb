...
require 'content_templates_controller'

# Re-raise errors caught by the controller.
class ContentTemplatesController; def rescue_action(e) raise e end; end
...
class ContentTemplatesControllerTest < ActionController::TestCase
  ...
  def setup
    @controller = ContentTemplatesController.new
    @request    = ActionController::TestRequest.new
    @response   = ActionController::TestResponse.new
    login
    @opts =  {:controller => "content_templates", :proj_id => "1"}
  end
