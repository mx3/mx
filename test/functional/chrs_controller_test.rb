...
require 'chrs_controller'

# Re-raise errors caught by the controller.
class ChrsController; def rescue_action(e) raise e end; end

class ChrsControllerTest < ActionController::TestCase
  fixtures :chrs, :chr_states

  def setup
    @request, @response = ActionController::TestRequest.new, ActionController::TestResponse.new
    @controller = ChrsController.new
    login
    @opts = {:controller => "chrs", :proj_id => "1"}
  end
  ...
  def test_route_to_index
    opts = {:controller => "chrs", :action => "index", :proj_id => "1"}
    assert_recognizes opts , 'projects/1/chrs'
    assert_routing "projects/1/chrs", opts
  end
